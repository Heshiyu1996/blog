...
## WebSocket特点
 ...
 4、**[性能开销小]** 数据格式比较轻量，性能开销小，通信高效；

 5、**[可发两类内容]** 可以发送文本、二进制（Blob、Arraybuffer）；

 6、**[可跨域]** 没有同源限制，客户端可以和任意服务器通信；

 7、**[可复用性]** 可复用长连接（如：心跳检测时的“长轮询”），同一条`WebSocket`上能同时并发多个请求；

![alt](./img/WebSocket-1.png)

## WebSocket缺点
 1、前、后端需要维护`WebSocket`的连接状态（心跳检测、断开重连）

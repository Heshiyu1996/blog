...
## HTTPS的实现原理
HTTPS 在证书验证上使用的是 `非对称加密`， 在 数据传输上 使用的是 `对称加密`。

`非对称加密`：浏览器和服务器同时拥有公钥和私钥（公钥用来加密、私钥用来解密）。公钥在网络中传输。
 - 优点：安全
...
### 证书验证阶段
 - 浏览器发起 HTTPS 请求
 - 服务端返回 HTTPS 证书（包含公钥）
 - 浏览器验证 HTTPS 证书是否合法（如果不合法提示警告）

### 数据传输阶段
 - 浏览器会生成一个 随机数（用作后续请求的“对称密钥”）
 - 通过 公钥 加密这个 随机数（即“对称密钥”） ，发送给服务端
 - 服务端通过 私钥 解密随机数（取出“对称密钥”）
 - 通过传入的 随机数（即“对称密钥”），对数据进行 对称加密

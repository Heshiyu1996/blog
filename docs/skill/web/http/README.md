...
## HTTP1.0如何复用TCP连接
 HTTP1.0默认都是`非持久连接`，每次请求都需建立一次TCP连接
  - 需要在HTTP请求头指定`connection: Keep-Alive`

 HTTP1.1默认连接是`持久连接`，客户端可以在一个TCP连接中发起多次HTTP请求

 HTTP2.0`多路复用`，客户端可以在一个TCP连接中并行请求，且不受**同一域名下请求个数的限制**
  - 所有请求都是基于流，最大并发可更高
